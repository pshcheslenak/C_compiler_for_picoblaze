    ...
    def move_data(self, target_spot, start_spot, size, reg, asm_code):
        ...
        while shift < size:
            ...
            if isinstance(start_spot, LiteralSpot):
                ...
                asm_code.add(asm_cmds.Load(reg, start_spot, reg_size))

            if reg != target_spot:
                asm_code.add(asm_cmds.Load(target_spot, reg, reg_size))
...
class Set(_ValueCmd):
    ...
    def make_asm(self, spotmap, home_spots, get_reg, asm_code): # noqa D102
        if self.output.ctype.weak_compat(ctypes.bool_t):
            ...
            asm_code.add(asm_cmds.Load(out_spot, arg_spot))
            ...
            asm_code.add(asm_cmds.Load(r, spotmap[self.arg]))

            # If necessary, move from r_asm -> output_asm
            if r != spotmap[self.output]:
                asm_code.add(asm_cmds.Load(spotmap[self.output], r))
    ...
    def _set_bool(self, spotmap, get_reg, asm_code):
        ...
        if (isinstance(spotmap[self.arg], LiteralSpot)
            ...
            asm_code.add(
                asm_cmds.Load(r, spotmap[self.arg]))
            arg_spot = r
        else:
            arg_spot = spotmap[self.arg]
        ...
        asm_code.add(asm_cmds.Load(output_spot, zero))
        asm_code.add(asm_cmds.Compare(arg_spot, zero))
        asm_code.add(asm_cmds.JumpZ(label))
        asm_code.add(asm_cmds.Load(output_spot, one))
...
class ReadAt(_ValueCmd):
    ...
    def make_asm(self, spotmap, home_spots, get_reg, asm_code):  # noqa D102
        ...
        if isinstance(addr_spot, RegSpot):
            ...
            asm_code.add(asm_cmds.Load(addr_r, addr_spot))
...
class _RelCommand(_ValueCmd):
    ...
    def get_rel_spot(self, spotmap, get_reg, asm_code):
        ...
        asm_code.add(asm_cmds.Load(r, spotmap[self.count]))
...
class AddrRel(_RelCommand):
    ...
    def make_asm(self, spotmap, home_spots, get_reg, asm_code):  # noqa D102
        ...
        if out_spot != spotmap[self.output]:
            asm_code.add(asm_cmds.Load(spotmap[self.output], out_spot))
